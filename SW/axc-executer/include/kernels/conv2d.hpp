...
namespace Exact {
/**
 * @brief Exact convolution implementation. It uses a complete spatial
            ...
            for (int c = 0; c < input_channels; ++c) {
              for (int i = 0; i < k_height; ++i) {
                for (int j = 0; j < k_width; ++j) {
                  ...
                  pos += c * k_height * k_width; /* move plane */
                  pos += map * k_height * k_width * input_channels;
...
/**
 * @brief Convolution 2D wrapper. It performs the validation and executes the
 * Conv2D kernel
 *
 * @tparam T Datatype
 ...
 * @return Runtime
 */
template <class T, class K = Exact::Conv2D<T>>
Runtime Conv2D(const BasicLayer *ilayer, const DataContainer *input,
               DataContainer *output) {
  K execution_kernel{};
  using LayerType = Layer<typename ::Conv2D>;
  using QLayerType = QLayer<T, typename ::Conv2D>;
  ...
  int k_maps = layer->props.dimensions[3];
