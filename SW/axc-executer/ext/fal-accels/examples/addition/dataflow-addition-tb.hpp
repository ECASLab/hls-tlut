...
#define EXECUTE_ACCEL                                                         \
  TOP_FUNCTION_NAME(                                                          \
      stream_input_a, stream_input_b, stream_output,                          \
      ACCESS_EXE_PARAM(elements), ACCESS_EXE_PARAM(load_streaming_mask),      \
      ACCESS_EXE_PARAM(en_accumulation), ACCESS_EXE_PARAM(en_activation),     \
      ACCESS_EXE_PARAM(scaling), ACCESS_EXE_PARAM(operation),                 \
      ACCESS_EXE_PARAM(execution_mode), ACCESS_CAP_RANGE(elements),           \
...
#define READ_PROPS                                                            \
  std::cout << "\nExecution params"                                           \
            << "\n Elements: " << READ_EXE_PARAM_TB(elements)                 \
            << "\n Load Matrix mask: " << std::hex                            \
            << READ_EXE_PARAM_TB(load_streaming_mask)                         \
            ...
            << "\nCapabilities"                                               \
            << "\n Min elements: " << ACCESS_CAP_MIN(elements)                \
            << "\n Max elements: " << ACCESS_CAP_MAX(elements)                \
...
#define DECL_EXE_PARAMS_TB                              \
  DECL_EXE_PARAM_TB(DimensionPort, elements);           \
  DECL_EXE_PARAM_TB(PropertyPort, load_streaming_mask); \
 ...
 */
#define DECL_CAPS_TB                             \
  DECL_RANGE_CAP_TB(DimensionPort, elements);    \
