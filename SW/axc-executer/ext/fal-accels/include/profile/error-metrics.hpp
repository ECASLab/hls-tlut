 ...
 */
std::pair<double, double> MeanStd(const cv::Mat matrix) {
  assert(!matrix.empty());
 ...
 */
std::pair<double, double> MinMax(const cv::Mat &matrix) {
...
double PSNR(const cv::Mat golden, const cv::Mat sample) {
  ...
  /* Find the max of the golden */
  auto min_max_ = MinMax(golden_float);
...
double Covariance(const cv::Mat x_p, const cv::Mat y_p) {
  ...
  /* Compute the means */
  auto mean_std_x = MeanStd(x);
  double mean_x = mean_std_x.first;
  auto mean_std_y = MeanStd(y);
...
/**
 * Computes the frobenius for real numbers
 * @param matrix matrix with the data
 * @return frobenius norm scalar
 */
double Frobenius(const cv::Mat matrix) {
  return std::sqrt(cv::trace(matrix * matrix.t())[0]);
}

/**
 * Computes the Frobenius distance between the matrices
 * @param ref matrix 1
 * @param exp matrix 2
 * @return the distance in double
 */
double FrobeniusDistance(const cv::Mat ref, const cv::Mat exp) {
  double frob_exp = Frobenius(exp);
  double frob_ref = Frobenius(ref);
  double frob_diff = Frobenius(cv::abs(ref - exp));

  return frob_diff / (frob_exp * frob_ref);
}
