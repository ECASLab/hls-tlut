...
/**
 * Standard property port for matrix operations.
 */
...
/**
 * Standard dimension port for matrix operations.
 */
using DimensionPort = ap_int<16>;

/**
 * Standard fulltype port for matrix operations.
 */
using FullType = ap_int<32>;

/**
 * Standard datatype port for matrix operations.
 */
using DataType = ap_fixed<Q_BW, Q_INT>;

#define TOP_MATRIX_FUNCTION(topname)                              \
  void topname(/* Streaming port */                               \
               ...
               DECL_EXE_PARAM(PropertyPort, load_matrix_mask),    \
 ...
 * @example
 * // Header file:
 * DECL_TOP_MATRIX_FUNCTION(myaccel);
 * \endcode
 * @param topname name of the top function
 */
#define DECL_TOP_MATRIX_FUNCTION(topname) TOP_MATRIX_FUNCTION(topname)
 ...
 * DECL_SET_MATRIX_CAPABILITIES (myaccel_caps);
 * DECL_SET_MATRIX_PARAMS_CHECK (myaccel_checker);
 *
 * // Definitions of the auxiliar functions
 *
 * BEGIN_DEF_TOP_MATRIX_FUNCTION (myaccel, myaccel_caps, myaccel_checker) {
 ...
 */
#define BEGIN_DEF_TOP_MATRIX_FUNCTION(topname, caps, exe_params)           \
  TOP_MATRIX_FUNCTION(topname) {                                           \
    /* -- Set capabilities -- */                                           \
    caps(ACCESS_CAP_RANGE(rows), ACCESS_CAP_RANGE(columns),                \
         ACCESS_CAP_RANGE(scaling), ACCESS_CAP(masking),                   \
         ACCESS_CAP(accumulation), ACCESS_CAP(activation),                 \
         ACCESS_CAP(def_scaling), ACCESS_CAP(operations),                  \
    ...
    exe_params(ACCESS_EXE_PARAM(rows), ACCESS_EXE_PARAM(columns),          \
               ACCESS_EXE_PARAM(load_matrix_mask),                         \
 ...
 * \endcode
 */
#define END_DEF_TOP_MATRIX_FUNCTION() }

#define SET_MATRIX_CAPABILITIES(funcname)                                      \
 ...
 */
#define DECL_SET_MATRIX_CAPABILITIES(funcname) SET_MATRIX_CAPABILITIES(topname)
 ...
 * DECL_SET_MATRIX_CAPABILITIES(myaccel_caps);
 *
 * DEF_SET_MATRIX_CAPABILITIES (myaccel_caps) {
 ...
 * @param funcname name of the function
 */
#define DEF_SET_MATRIX_CAPABILITIES(funcname) SET_MATRIX_CAPABILITIES(funcname)

#define SET_MATRIX_PARAMS_CHECK(funcname)                                     \
  inline static void funcname(DECL_EXE_PARAM(DimensionPort, rows),            \
                              DECL_EXE_PARAM(DimensionPort, columns),         \
                              DECL_EXE_PARAM(PropertyPort, load_matrix_mask), \
                              DECL_EXE_PARAM(PropertyPort, en_accumulation),  \
                              DECL_EXE_PARAM(PropertyPort, en_activation),    \
 ...
 * \code{.c}
 * // Header file:
 * DECL_SET_MATRIX_PARAMS_CHECK(myaccel_caps);
 * \endcode
 * @param funcname name of the function
 */
#define DECL_SET_MATRIX_PARAMS_CHECK(funcname) SET_MATRIX_PARAMS_CHECK(funcname)
 ...
 * DECL_SET_MATRIX_PARAMS_CHECK (myaccel_checker);
 *
 * DEF_SET_MATRIX_CAPABILITIES (myaccel_checker) {
 ...
 * @param funcname name of the function
 */
#define DEF_SET_MATRIX_PARAMS_CHECK(funcname) SET_MATRIX_PARAMS_CHECK(funcname)
