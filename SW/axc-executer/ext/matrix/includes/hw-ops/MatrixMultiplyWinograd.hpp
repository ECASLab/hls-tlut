...
class MatrixMultiplyWinograd : public MatrixOperator<T, M, N, ADD, MULT, NL> {
 public:
  /**
...
inline void MatrixMultiplyWinograd<T, M, N, ADD, MULT, NL>::Execute(
    const T op_a[MatrixOperator<T, M, N, ADD, MULT, NL>::rows]
                [MatrixOperator<T, M, N, ADD, MULT, NL>::columns],
    const T op_b[MatrixOperator<T, M, N, ADD, MULT, NL>::columns]
                [MatrixOperator<T, M, N, ADD, MULT, NL>::rows],
  ...
  op_c[0][0] = ata + b1 * b2;
  op_c[0][1] = w + v + (a1 + b1 - cpd) * d2;
  op_c[1][0] = wpu + d1 * (b2 + cmamd);
  op_c[1][1] = wpu + v;
}
}  // namespace operators
}  // namespace hw
