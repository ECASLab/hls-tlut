...
static void TanhTlut(const T *input_data, T *output_data, const int size) {
  std::cout << "\n========================================================================================" << std::endl;
  std::cout << "[DEBUG TLUT] Entrando a TanhTlut | Tamaño del lote: " << size << std::endl;
  std::cout << "========================================================================================" << std::endl;
  ...
  std::cout << "\n[DEBUG TLUT] MUESTRAS DE ENTRADA (Host -> FPGA):" << std::endl;
  std::cout << "  Idx   | Valor Real (float) | Bits Hex  | Bits Dec (signed) | Tamaño T" << std::endl;
  std::cout << "  ------|--------------------|-----------|-------------------|-----------" << std::endl;
  for (int i = 0; i < std::min(size, 5); ++i) {
      float valor_humano = static_cast<float>(input_data[i]);
      uint16_t bits_puros = input_data[i].V;
      int16_t bits_signed = static_cast<int16_t>(bits_puros); // Interpretación con signo para el humano
      
      printf("  [%3d] | %18.6f | 0x%04x    | %17d | %d bytes\n", 
             i, valor_humano, bits_puros, bits_signed, (int)sizeof(input_data[i]));
  }
  ...
  std::cout << "\n[DEBUG TLUT] MUESTRAS DE SALIDA (FPGA -> Host):" << std::endl;
