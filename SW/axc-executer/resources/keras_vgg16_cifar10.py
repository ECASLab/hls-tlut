...
# this is done so the weights from imagenet don't get overwritten
base_model.trainable = False

base_model.summary()

# need these because the default top 3 layers of the VGG16 in keras don't
# work, when running model.fit(), the accuracy doesn't change between epochs
...
for i, zipped in enumerate(zip(_sample_imgs, _sample_labels)):
    img, label = zipped
    plt.imsave(
        f"img-{i}_label-{label}.png", img, pil_kwargs={"aspect": "equal"}
    )
    print(f"\nlabel: {label}")

    ym = model(np.array([img.reshape(-1, 32, 3)]))
    print(f"ym: {ym}")
